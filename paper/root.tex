...
Existing mitigations fall into three categories (\S II): adaptation methods~\cite{obeyed} fine-tune attention adapters, requiring architecture-specific retraining; inference-time interventions such as BYOVLA~\cite{byovla} identify distractors with a VLM (an external GPT-4o dependency) and probe per-region sensitivity with multiple VLA forward passes, with threshold-based target protection; and training-time augmentation~\cite{rosie, genaug, nice} generates cluttered training data at the cost of retraining, without deployment-time guarantees.
...
VLA models such as RT-2 \cite{rt2}, OpenVLA \cite{openvla}, and Octo \cite{octo} leverage internet-scale vision-language pre-training for remarkable zero-shot generalization, yet struggle with high-precision geometric grounding in cluttered scenes~\cite{distracted_robot, eva_vla}. Contributing factors include the static patch resolution of ViT backbones \cite{vit_analysis} and the absence of coarse-to-fine zooming \cite{c2f_q_attention}; we hypothesize that these properties make such architectures susceptible to feature dilution \cite{distracted_robot}, where background clutter competes with task-relevant signals for representational capacity.

\subsection{Vision Foundation Models for Robotic Perception}
Vision Foundation Models (VFMs) such as SAM~3 \cite{sam3} and GroundingDINO \cite{groundingdino} localize objects from free-form text. Robotics has mostly used them to \textit{add} information---3D value maps \cite{voxposer}, semantic maps \cite{conceptfusion}, dataset labeling \cite{robogen}, visual prompting \cite{RoboticVisualInstruction}. We invert this paradigm, using their open-set discriminative power to \textit{suppress} task-irrelevant regions: an information bottleneck in an informal, analogical sense~\cite{BC-IB}---unlike \cite{BC-IB}, there is no learned rate--distortion trade-off, only a hard, hand-specified pixel-space mask.

\subsection{Robustness via Adaptation and Training-Time Augmentation}
Clutter robustness has traditionally relied on domain randomization \cite{Tobin2017} and large-scale pre-training \cite{bridgedata, open_x_embodiment}, yet VLAs remain fragile to cluttered-scene shifts \cite{colosseum, robocasa}. Adaptation methods such as OBEYED-VLA \cite{obeyed} train attention layers to recover grounding, at the cost of architecture-specific retraining. Training-time augmentation generates cluttered or counterfactual data: ROSIE \cite{rosie}, GenAug \cite{genaug}, and NICE \cite{nice}, whose pixel-space \emph{scene surgery} is the closest prior to our core operation. CGVD performs a related pixel-space edit, but at \emph{inference} time on a frozen policy: no retraining, instruction-adaptive, at the price of inheriting run-time perception risk that training-time methods amortize into the weights.

\subsection{Inference-Time Attention Intervention}
Distracting Token Pruning (DTP) \cite{dtp} suppresses text-misaligned visual tokens in feature space; we conjecture such pruning struggles when distractor and target tokens are already entangled by early self-attention, and instead intervene in pixel space, before tokenization. Unlike Visual Prompting \cite{RoboticVisualInstruction}, which adds information to guide attention, we remove information, preventing attention leakage to distractors. BYOVLA \cite{byovla} also intervenes on the observation at run time; \S\ref{sec:setup} states why no head-to-head comparison is reported here.
...
\begin{figure}[t]
    \centering
    % --- Top Image: Baseline ---
    \includegraphics[width=\linewidth]{figures/base_line_att_map_2panel.png}

    \vspace{0.1cm} % Small gap between the images

    % --- Bottom Image: CGVD ---
    \includegraphics[width=\linewidth]{figures/cgvd_att_map_2panel.png}

    % --- The Single Caption ---
    \caption{\textbf{Qualitative illustration of the attention-repair hypothesis} (policy input and attention overlay).
    (Top) Under clutter, the baseline policy's attention is dispersed across distractors rather than the spoon.
    (Bottom) With CGVD, attention concentrates more on the target region.
...
Table~\ref{tab:combined_attribute_scaling} reveals a two-sided dependence on descriptive density. On the baseline side, performance with simple adjective-noun prompts degrades only mildly with clutter (87.0\% $\rightarrow$ 75.0\%), but collapses on compositional queries (86.0\% $\rightarrow$ 57.0\%), confirming the bag-of-words brittleness. On the CGVD side, the complex prompt is what unlocks the benefit: CGVD holds a stable floor under the compositional query (73.0\% at 4 distractors, $\Delta = +16.0$, the one delta in this table whose CI excludes zero), because SAM~3's open-set grounding exploits the rich attribute phrase to isolate the correct instance. With the \emph{simple} prompt, by contrast, CGVD provides no reliable benefit (deltas $-5.0$ to $+3.0$, all within noise): the terse phrase ``green spoon'' does not give the segmenter enough descriptive signal to separate the target from attribute-conflicting spoons. The bottleneck is the segmentation prompt itself: re-running the simple-prompt arm with the scene-informed descriptive target (``spoon with green handle'') supplied to SAM~3 restores large gains at density (CGVD 87.0/82.0/83.0 at 2--4 distractors vs.\ 79.0--70.0 with the terse target)---but that phrasing encodes scene knowledge (\emph{where} the green is) that is not derivable from the simple instruction, so we report the instruction-consistent numbers as primary. CGVD thus converts descriptive density into robustness; it does not rescue a prompt that lacks it. Given the 5-seed sample, we read the per-cell values in this table as descriptive.
...
\textbf{Simulation-only evaluation and privileged inputs.} No real-robot experiment is reported, and two pipeline inputs are simulation-privileged: the renderer robot mask (\S\ref{sec:compositing}) and the per-episode exact distractor inventory $\mathcal{D}$ (\S\ref{sec:decomposition}). Sim-to-real risk is therefore not limited to policy transfer: SAM~3 recall over $\mathcal{D}$ and LaMa fill fidelity on real textured scenes (the largest single factor in Table~\ref{tab:ablation}) are unvalidated on hardware; for robot masking, the deployable per-frame SAM~3 substitute is measured in simulation (\S\ref{sec:ablation}: accuracy-neutral, but slow and unreliable per frame), while FK projection on hardware remains untested.
