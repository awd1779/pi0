...
\textbf{Distractors.} Following the distractor-type framing of \cite{distracted_robot}, we evaluate three types: (1) \textit{Semantic}: objects with high semantic proximity and shared visual characteristics with the target; (2) \textit{Random}: arbitrary objects with no semantic or visual similarity; and (3) \textit{Attribute}: objects sharing the same category and form as the target but differing in physical properties. Distractor assets are drawn from RoboCasa~\cite{robocasa} and the YCB~\cite{ycb} dataset and placed with collision-aware grid-based placement on a fixed 6\,cm grid (one object per cell); when an object cannot be placed---cells exhausted at high densities---it is placed at a collision-free random position, or hidden off-scene if none exists, rather than the episode being dropped. The CGVD distractor vocabulary $\mathcal{D}$ is re-derived per episode from the category labels of the actually-spawned assets (e.g., \texttt{rc\_fork\_0} $\rightarrow$ ``fork''), i.e., $\mathcal{D}$ coincides exactly with each episode's injected inventory (\S\ref{sec:decomposition}).
