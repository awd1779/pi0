...
\textbf{Occlusion fabrication and in-flight blending.} Because Eq.~\ref{eq:inpaint_mask} inpaints the robot region at $t{=}0$, whatever the arm occluded in the initialization frame is synthesized, not observed, in the cached clean scene for the entire episode. Relatedly, the compositing mask $\alpha$ protects the target only at its $t{=}0$ location: auditing the headline condition against ground truth, the \emph{moving} target's pixels overlap previously inpainted regions by more than 10\% in 35\% of episodes at some point during execution, exposing the grasped object to partial blending toward the cached background.

\textbf{Startup latency.} The single-frame initialization introduces a $0.7$--$3.3$\,s delay (scene-complexity dependent) before the first action of each episode (Table~\ref{tab:latency}).
