...
\textbf{Simulation-only evaluation and privileged inputs.} No real-robot experiment is reported, and two pipeline inputs are simulation-privileged: the renderer robot mask (\S\ref{sec:compositing}) and the per-episode exact distractor inventory $\mathcal{D}$ (\S\ref{sec:decomposition}). Sim-to-real risk is therefore not limited to policy transfer: SAM~3 recall over $\mathcal{D}$, LaMa fill fidelity on real textured scenes (the largest single factor in Table~\ref{tab:ablation}), and per-frame robot masking are all unvalidated on hardware.
