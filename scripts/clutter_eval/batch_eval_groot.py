...
class GR00TBatchEvaluator:
    ...
    def __init__(
        ...
        if embodiment == "bridge":
            self.env_adapter = GR00TBridgeSimplerAdapter(image_size=(224, 224))
        elif embodiment == "fractal":
            self.env_adapter = GR00TFractalSimplerAdapter(image_size=(224, 224))
        else:
    ...
    # ------------------------------------------------------------------
    # Output helpers (same format as batch_eval.py for direct comparison)
    # ------------------------------------------------------------------
...
def main():
    ...
    parser.add_argument("--act_steps", type=int, default=4,
                       help="Number of action steps to execute per inference (default: 4)")
    ...
    if args.model_path is None:
        if args.task.startswith("widowx"):
            ...
            args.model_path = "nvidia/GR00T-N1.6-3B"
        else:
