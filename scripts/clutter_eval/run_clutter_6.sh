#!/bin/bash
# Clutter evaluation with 6 distractor objects (with CGVD)
# Usage: ./run_clutter_6.sh [NUM_EPISODES]
...
mkdir -p $LOG_DIR
LOG_FILE="${LOG_DIR}/cgvd_clutter_6_${TIMESTAMP}.log"

echo "Running CGVD evaluation with 6 distractors, $NUM_EPISODES episodes"
...
# Run evaluation and tee output to both console and log file
{
    ...
    xvfb-run -a -s "-screen 0 1024x768x24" uv run python scripts/try_checkpoint_in_simpler.py \
        ...
        --recording \
        --output_dir videos/cgvd_clutter_6 \
        --use_bf16 \
        --use_cgvd \
        --cgvd_update_freq 1 \
