#!/bin/bash
# Baseline clutter evaluation with 4 distractor objects (NO CGVD)
# Usage: ./run_clutter_4_baseline.sh [NUM_EPISODES]
...
mkdir -p $LOG_DIR
LOG_FILE="${LOG_DIR}/baseline_clutter_4_${TIMESTAMP}.log"

echo "Running BASELINE evaluation with 4 distractors, $NUM_EPISODES episodes (NO CGVD)"
...
# Run evaluation and tee output to both console and log file
{
    ...
    xvfb-run -a -s "-screen 0 1024x768x24" uv run python scripts/try_checkpoint_in_simpler.py \
        ...
        --recording \
        --output_dir videos/baseline_clutter_4 \
        --use_bf16
