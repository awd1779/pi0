#!/usr/bin/env python3
"""Batch evaluation script for SpatialVLA on SimplerEnv with full sweep support.

This is the SpatialVLA equivalent of batch_eval_openvla.py. It loads the SpatialVLA
model ONCE and runs through ALL configurations (categories x distractor_counts x runs),
providing the same output format for direct comparison with Pi0, GR00T, and OpenVLA.

SpatialVLA uses action chunking (4 actions per inference) and image history.

Must be run in the 'spatialvla' conda environment.

Usage:
    # Full sweep
    conda run -n spatialvla python scripts/clutter_eval/batch_eval_spatialvla.py \
...
from src.model.vla.spatialvla import SpatialVLAInference
from src.agent.env_adapter.spatialvla_simpler import SpatialVLABridgeSimplerAdapter


# ---------------------------------------------------------------------------
# Data classes (identical to batch_eval.py for output compatibility)
# ---------------------------------------------------------------------------
...
class EvalConfig:
    ...
    randomize_distractors: bool = False
    distractor_pool_size: int = 0
...
class SpatialVLABatchEvaluator:
    """Evaluator that loads SpatialVLA once and runs multiple configurations."""
    ...
    def __init__(
        ...
        # Load model ONCE
        print("\n" + "=" * 70)
        print("SpatialVLA BATCH EVALUATOR: Loading model (shared across all configurations)")
        print("=" * 70)
        model_load_start = time.time()

        self.model = SpatialVLAInference(
        ...
        self.env_adapter = SpatialVLABridgeSimplerAdapter(image_size=(224, 224))
    ...
    def _run_episode(
        ...
        while True:
            # SpatialVLA inference: preprocess -> forward -> postprocess
            ...
            # actions is [T, 7] chunk, postprocess all
            env_actions = self.env_adapter.postprocess(actions)

            # Execute act_steps actions from the chunk
    ...
    def run_configuration(self, config: EvalConfig, config_output_dir: Optional[str] = None) -> ConfigResult:
        ...
        print(f"  Results: Baseline={result.baseline_rate:.1f}%, CGVD={result.cgvd_rate:.1f}%, D={result.improvement:+.1f}%")
        return result
    ...
    def _save_log_file(self, log_lines: List[str], results: List[EpisodeResult],
        ...
        with open(log_path, "w", encoding="utf-8") as f:
            f.write(f"Task: {config.task}\n")
            f.write(f"Mode: {mode}\n")
            f.write(f"Model: spatialvla ({self.model_path})\n")
            f.write(f"Act steps: {self.act_steps}\n")
            f.write(f"Seed: {config.seed}\n")
    ...
    def _save_aggregated_report(self, results: List[ConfigResult], output_dir: str):
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            f.write("# Paired Evaluation Report (SpatialVLA)\n\n")
            f.write(f"**Generated:** {time.strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            f.write("## Configuration\n\n")
            f.write("| Parameter | Value |\n")
            f.write("|-----------|-------|\n")
            f.write(f"| Model | SpatialVLA-4B |\n")
            ...
            f.write("| Run | Seed | Baseline SR | CGVD SR | D SR | Baseline h-SR | CGVD h-SR | D h-SR |\n")
            f.write("|-----|------|-------------|---------|------|---------------|-----------|--------|\n")
            ...
            f.write("| Method | Mean | Std | Min | Max |\n")
            f.write("|--------|------|-----|-----|-----|\n")
            f.write(f"| Baseline | {baseline_hard_mean:.1f}% | {np.std(baseline_hard_rates):.1f}% | {min(baseline_hard_rates):.1f}% | {max(baseline_hard_rates):.1f}% |\n")
            f.write(f"| CGVD | {cgvd_hard_mean:.1f}% | {np.std(cgvd_hard_rates):.1f}% | {min(cgvd_hard_rates):.1f}% | {max(cgvd_hard_rates):.1f}% |\n\n")
    ...
    def run_sweep(
        ...
        # Pre-compute output directories
        config_dirs: Dict[Tuple[str, int], str] = {}
        if output_dir:
...
# ---------------------------------------------------------------------------
# Config generation (same logic as batch_eval.py)
# ---------------------------------------------------------------------------
...
def save_sweep_results(results: List[ConfigResult], output_dir: str, model_path: str):
...
def print_sweep_summary(results: List[ConfigResult]):
    ...
    print("\n" + "=" * 120)
    print("SpatialVLA SWEEP SUMMARY")
    print("=" * 120)
    ...
    for (category, num_dist), group in sorted(grouped.items()):
        ...
        baseline_hard_mean = np.mean([r.baseline_hard_success_rate for r in group])
        cgvd_hard_mean = np.mean([r.cgvd_hard_success_rate for r in group])
...
def main():
    parser = argparse.ArgumentParser(description="SpatialVLA batch evaluation for configuration sweeps")

    # Task configuration
    parser.add_argument("--task", type=str, default="widowx_spoon_on_towel")
    parser.add_argument("--model_path", type=str, default="IPEC-COMMUNITY/spatialvla-4b-224-sft-bridge",
                       help="SpatialVLA model path (HuggingFace or local)")
    parser.add_argument("--unnorm_key", type=str, default="bridge_orig/1.0.0",
                       help="Unnormalization key for action decoding")
    parser.add_argument("--gpu_id", type=int, default=0)
    parser.add_argument("--act_steps", type=int, default=4,
                       help="Number of action steps to execute per inference (default: 4, action chunking)")
    ...
    print("=" * 70)
    print("SpatialVLA BATCH EVALUATION CONFIGURATION")
    print("=" * 70)
    print(f"Model: SpatialVLA-4B ({args.model_path})")
    ...
    if args.output_dir is None:
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        args.output_dir = f"logs/clutter_eval/spatialvla/{args.task}/{timestamp}"

    # Run batch evaluation
    evaluator = SpatialVLABatchEvaluator(
    ...
    # Save and summarize
    save_sweep_results(results, args.output_dir, args.model_path)
