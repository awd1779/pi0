...
def plot_panel(ax, data: dict, show_xlabel: bool, show_ylabel: bool):
    ...
    ax.fill_between(x, bl_m - bl_se, bl_m + bl_se, color=COLOR_BASELINE, alpha=0.12)
    ax.fill_between(x, cg_m - cg_se, cg_m + cg_se, color=COLOR_CGVD, alpha=0.12)

    # Lines
    ax.plot(x, bl_m, "o-", color=COLOR_BASELINE, markerfacecolor="white",
            markeredgewidth=1.2, zorder=3, label="$\\pi_0$")
    ax.plot(x, cg_m, "s-", color=COLOR_CGVD, markerfacecolor="white",
